...
from tabdiff.utils import set_random_seed

def main(args: argparse.Namespace):
    # 1) Load user‐edited TOML
    this_dir    = os.path.dirname(__file__)
    config_path = os.path.join(this_dir, "configs", "tabdiff_configs.toml")
    raw_cfg     = OmegaConf.load(config_path)

    # ─────────────────────────────────────────────────────────────────────────
    # NOTE: We REMOVE any hard‐coded override of check_val_every here,
    # so your toml’s `train.main.check_val_every = 9999` (or whatever) is used.
    # ─────────────────────────────────────────────────────────────────────────
    ...
    if args.mode == "train":
        ...
        metrics, df, _ = trainer.sample_and_report(
            ...
            num_samples=config.sample.batch_size,
